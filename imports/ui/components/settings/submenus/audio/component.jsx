...
class SettingInputStreamLiveSelector extends Component {
    ...
    const deviceList = (listLenght > 0)
      ? list.map((device) => (
        <Dropdown.DropdownListItem
          ...
          label={SettingInputStreamLiveSelector.truncateDeviceName(device.label)}
  ...
  render() {
    ...
    return (
        <>
            { (audioInputDevices!=null && audioOutputDevices!=null) ? 
                <>
                <h3 className={styles.title}>
                    Select Audio Device
                </h3>
                <div className={styles.audiosel}>

                    <select
                    id="setCam"
                    value={selectedInputDeviceId || currentInputDeviceId}
                    className={styles.aselect}
                    onChange={(e) => this.onDeviceListClick(e.target.value, AUDIO_INPUT,
                        liveChangeInputDevice)}
                    >
                        {audioInputDevices.map((webcam, index) => (
                    ...
                    <select
                    id="setCam2"
                    value={selectedOutputDeviceId || currentOutputDeviceId}
                    className={styles.aselect}
                    //onChange={(e)=>{this.liveChangeOutputDevice}}
                    onChange={(e) => this.onDeviceListClick(e.target.value, AUDIO_OUTPUT,
                        liveChangeOutputDevice)}
                    >
                        {audioOutputDevices.map((webcam, index) => (
                            <option key={webcam.deviceId} value={webcam.deviceId}>
                            {webcam.label || this.getFallbackLabel(webcam, index)}
                            </option>
                        ))}
...
SettingInputStreamLiveSelector.propTypes = propTypes;

export default withShortcutHelper(injectIntl(SettingInputStreamLiveSelector),
  ['leaveAudio']);
